...
def extract_search_params_with_llm(natural_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        prompt = f"""
以下のユーザーの自然言語クエリから、Jグランツ補助金検索APIに必要な検索パラメータを抽出してください。

ユーザーのクエリ: "{natural_query}"

【重要】
- keyword はユーザーが探している主要な概念（名詞）を抽出し、2〜10文字程度の短いキーワードにまとめてください（文章をそのまま入れない）。
- industry / target_area_search / target_number_of_employees / use_purpose は、ユーザーが明確に言っていなくても、文脈から合理的に推測して抽出してください。
- 林業の助成金を探している → industry = 林業
- 製造業の企業が使える補助金 → industry = 製造業
- IT導入の補助金を探している → use_purpose = 設備整備・IT導入
- DX, デジタル化 → 設備整備・IT導入
- 環境, 脱炭素 → エコ・SDGs活動支援
...
【回答フォーマット（JSONのみ）】
{
  "keyword": "抽出した主要キーワード（必須）",
  "industry": "推定した業種（該当する場合のみ）",
  "target_number_of_employees": "従業員数（該当する場合のみ）",
  "target_area_search": "対象地域（該当する場合のみ）",
  "use_purpose": "利用目的（該当する場合のみ）"
}

【注意】
- 当てはまらない項目は JSON に含めないでください。
- 文章の意味から自然言語推論を行い、最も妥当な業種・目的を抽出してください。
- keyword は必ず含めてください。
"""
